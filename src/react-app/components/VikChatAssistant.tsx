...
export function VikChatAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Namaste! Main VikChat Assistant hun 👋 Features samjhana ho, chat shuru karni ho, ya koi sawaal ho — main yahan hun!",
    },
  ]);
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setMessages([...newMessages, { role: "assistant", content: "Kuch error aa gaya. Dobara try karo!" }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
        ...
        <div style={{
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{
                  ...
                  background: msg.role === "user" ? "linear-gradient(135deg,#C9A84C,#E8C97A)" : "rgba(201,168,76,0.08)",
          ...
          {messages.length <= 1 && (
            <div style={{ padding: "4px 14px 8px", display: "flex", gap: "6px", flexWrap: "wrap" }}>
              {quickBtns.map((btn) => (
                <button key={btn} onClick={() => sendMessage(btn)} style={{
                  background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.18)",
                  color: "rgba(201,168,76,0.8)", fontSize: "11px", padding: "4px 10px",
                  borderRadius: "20px", cursor: "pointer", fontFamily: "'DM Sans', sans-serif",
                }}>{btn}</button>
              ))}
            </div>
          )}

          <div style={{ padding: "10px 14px", borderTop: "1px solid rgba(201,168,76,0.1)", display: "flex", gap: "8px", alignItems: "center" }}>
            <input
              ref={inputRef}
              value={input}
      ...
      <style>{`@keyframes vcBounce { 0%,60%,100%{transform:translateY(0)} 30%{transform:translateY(-5px)} }`}</style>
    </>
  );
}
