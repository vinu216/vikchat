...
app.post("/api/assistant", async (c) => {
  try {
    ...
    if (!apiKey) return c.json({ content: [{ type: "text", text: "Service unavailable." }] }, 200);
    ...
    if (data.error) return c.json({ content: [{ type: "text", text: "Kuch error aa gaya. Thodi der baad try karo!" }] }, 200);
    return c.json(data);
  } catch (err) {
    return c.json({ content: [{ type: "text", text: "Server error. Dobara try karo!" }] }, 200);
  }
});
